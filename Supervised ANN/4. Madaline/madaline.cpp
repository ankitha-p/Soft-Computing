...
int main()
{
    ...
    for(int i=1;i<=noInputs;i++) cout<<"x"<<i<<"  ";
    cout<<"a  t  ";
    for(int i=1;i<=noInputs;i++) cout<<"Zin"<<i<<"  ";
    for(int i=1;i<=noInputs;i++) cout<<"Z"<<i<<"  ";
    cout<<"yin  y  ";
    for(int j=1;j<=noInputs;j++){
        for(int i=1;i<=noInputs;i++)
            cout<<"w"<<i<<j<<"  ";
    ...
    do{ /*for each input target pair:: In x[a][b] a represents the serial number,
        ...
        if(target[k]!=y)
        {
            if(target[k]==1)
            {
                ...
                int index=noInputs;
                for(int j=0;j<noInputs;j++) if(fabs(zin[j])<nearest) {nearest=zin[j];index=j;}
                bias[index]+=(a*(1-nearest));
                for(int i=0;i<noInputs;i++) weights[i][j]+=((a*(1-zin[j]))*x[i][j]);
            }
            else
            {
                for(int j=0;j<noInputs;j++)
                {
                    if(zin[j]>0) //update weights on units z, which have positive net input
                    {
                        bias[j]+=(a*(-1-zin[j]));
                        for(int i=0;i<noInputs;i++) weights[i][j]+=(a*(-1-zin[j])*x[i][j]);
                    }
        ...
        for(int i=0;i<noInputs;i++) cout<<x[k][i]<<"  ";
        cout<<"1  "<<target[k]<<"  ";
        for(int i=0;i<noInputs;i++) cout<<zin[i]<<"  ";
        for(int i=0;i<noInputs;i++) cout<<z[i]<<"  ";
        cout<<yin<<"  "<<y<<"  ";
        for(int j=0;j<noInputs;j++){
            for(int i=0;i<noInputs;i++)
                cout<<weights[i][j]<<"  ";
